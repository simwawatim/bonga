...
from pdf_invoice.database import UpdateRecieptUrl
...
class InvoicePDF:
    ...
    def draw_items_table(self, c, width, start_y):
        ...
        for idx, item in enumerate(self.invoice_data["items"], start=1):
            ...
            data.append([str(idx), item["name"], str(item["qty"]),
                         f"{item['price']:.2f}", f"{total_value:.2f} {currency}", item.get("tax_type","Standard Rated")])
    ...
    def draw_totals(self, c, width, start_y):
        ...
        y = start_y
        tax_categories = [("A", "Taxable Standard Rated"), ("B", "Taxable MTV"), ("C1", "Taxable C1"),
                          ("C2", "Taxable C2"), ("C3", "Taxable C3"), ("D", "Taxable D"),
                          ("E", "Taxable E"), ("F", "Taxable F"), ("Ipl1", "Taxable Import Level 1"),
                          ("Ipl2", "Taxable Import Level 2"), ("Tl", "Tourism Levy"),
                          ("Ecm", "Electronic Commerce"), ("Exeeg", "Exempt EG"), ("Rvat", "Reverse VAT")]
    ...
    def build_pdf(self, invoice_name, site_folder=None, site_name="media"):
        if site_folder is None:
            site_folder = os.path.join(os.getcwd(), site_name)
        output_folder = os.path.join(site_folder, "invoice")
        os.makedirs(output_folder, exist_ok=True)

        filename = str(uuid.uuid4()) + ".pdf"
        file_path = os.path.join(output_folder, filename)
        public_url = f"{self.site_url}/invoice/{filename}"
